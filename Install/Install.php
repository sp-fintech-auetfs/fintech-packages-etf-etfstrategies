...
namespace Apps\Fintech\Packages\Etf\Strategies\Install;

use Apps\Fintech\Packages\Etf\Strategies\Install\Schema\EtfStrategies;
use Apps\Fintech\Packages\Etf\Strategies\EtfStrategies as EtfStrategiesPackage;
...
class Install extends BasePackage
{
    ...
    public function init()
    {
        $this->databases =
            [
                'apps_fintech_etf_strategies'  => [
                    'schema'        => new EtfStrategies,
                    'model'         => new AppsFintechEtfStrategies
                ]
            ];
    ...
    {
        $strategiesArr = $this->basepackages->utils->scanDir('apps/Fintech/Packages/Etf/Strategies/Strategies/');

        if (count($strategiesArr['files']) > 0) {
            $strategiesPackage = new EtfStrategiesPackage;
            ...
            foreach ($strategiesArr['files'] as $key => $strategies) {
                ...
                $dbStrategy = $strategiesPackage->getEtfStrategiesByName($strategiesReflection->getShortName());
